...
async function fetchCraftyData(
  uuid: string
): Promise<{ data?: CraftyData } | null> {
  try {
...
async function processAltAccount(
  input: string,
  mainPlayerUuid: string
): Promise<string | null> {
  try {
    ...
    if (uuidRegex.test(input)) {
      ...
      const existingPlayerQuery = await getDocs(
        query(playersRef, where("uuid", "==", normalizedInput))
      );
      if (!existingPlayerQuery.empty) {
        const existingData =
          existingPlayerQuery.docs[0].data() as PlayerDocument;
      ...
      try {
        ...
        const player = searchData.data.find(
          (p) => p.username.toLowerCase() === input.toLowerCase()
        );
        if (!player) {
          return null;
        }
        ...
        // Check if player already exists
        const existingPlayerQuery = await getDocs(
          query(playersRef, where("uuid", "==", normalizedFoundUuid))
        );
        if (!existingPlayerQuery.empty) {
...
export async function updatePlayerData(
  ...
  try {
    ...
    if (existingPlayer && existingData?.mainAccount) {
      console.warn(
        `Cannot update ${existingData.currentIgn} as it is an alt account`
      );
      return;
    }
    ...
    const validAltUuids = processedAltUuids.filter(
    ...
    const existingPastIgns = (existingData?.pastIgns || []).map(
      (ign: PastIgn | string) =>
    ...
    for (const doc of mainAccountQuery.docs) {
      ...
      await updateDoc(doc.ref, {
        altAccounts: docData.altAccounts.filter(
          (id: string) => id !== normalizedUuid
        ),
      });
    }
  } catch (error) {
...
export async function updatePlayerEvents(
  ...
  try {
    ...
    if (playerDoc.exists()) {
      ...
      console.log(
        `Successfully ${action}ed event ${eventId} for player ${playerData.currentIgn} (${playerId})`
      );
    } else {
    ...
    if (playerDoc.exists()) {
      ...
      if (playerData.altAccounts?.length > 0) {
        ...
        for (const altUuid of playerData.altAccounts) {
          const altQuery = await getDocs(
            query(playersRef, where("uuid", "==", altUuid))
          );
          if (!altQuery.empty) {
      ...
      if (playerData.mainAccount) {
        const mainQuery = await getDocs(
          query(
            ...
            where("uuid", "==", playerData.mainAccount)
          )
        );
        if (!mainQuery.empty) {
          ...
          await updateDoc(mainDoc.ref, {
            altAccounts: mainData.altAccounts.filter(
              (id: string) => id !== playerData.uuid
            ),
          });
        }
      }
    ...
    if (!playerQuery.empty) {
      ...
      if (playerData.altAccounts?.length > 0) {
        for (const altUuid of playerData.altAccounts) {
          ...
          if (!altQuery.empty) {
            ...
            igns.push(
              ...altData.pastIgns.map((ign: PastIgn | string) =>
                typeof ign === "string" ? ign : ign.name
              )
            );
          }
      ...
      if (playerData.mainAccount) {
        ...
        if (!mainQuery.empty) {
          ...
          igns.push(
            ...mainData.pastIgns.map((ign: PastIgn | string) =>
              typeof ign === "string" ? ign : ign.name
            )
          );
        }

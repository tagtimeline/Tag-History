...
const VideoPlayer: React.FC<{ platform: string; id: string }> = ({
  platform,
  id,
}) => {
  const getUrl = () => {
    ...
    if (isMedia && hasDirectSubtext) {
      ...
      groups.push(
        <div
          ...
          className={
            isVideoContent ? styles.videoSubtextGroup : styles.imageSubtextGroup
          }
          style={{
            width: isImage(line)
      ...
      if (currentGroup.length > 0) {
        groups.push(
          <div
            ...
            className={formatStyles.paragraph}
          >
            {currentGroup}
          </div>
        );
...
const EventContent: React.FC<EventContentProps> = ({
  ...
  const [expandedSideEvents, setExpandedSideEvents] = useState<
  ...
  const usedSideEvents = useMemo(() => {
    ...
    return new Set(
      sideEventMatches.map((match) => parseInt(match.slice(6, -1)))
    );
  }, [event.description]);

  const renderSideEvent = useCallback(
    (sideEvent: NonNullable<TimelineEvent["sideEvents"]>[number]) => {
      return (
          ...
          {expandedSideEvents[sideEvent.id] && (
            <div className={styles.sideEventContent}>
              {(() => {
                ...
                const markers =
                  sideEvent.description.match(/\[TABLE-\d+\]/g) || [];
                ...
                return parts.map((part, index) => {
                  ...
                  if (index < markers.length) {
                    ...
                    if (tableMatch && event.tables) {
                      // Use event.tables instead of sideEvent.tables
                      const tableIndex = parseInt(tableMatch[1]);

                      if (tableIndex < event.tables.length) {
                        return (
    ...
    [expandedSideEvents, event.tables]
  ); // Make sure to include event.tables in dependencies
  ...
  const renderContent = () => {
    ...
    const markers =
      event.description.match(/\[(?:TABLE-\d+|SIDE-\d+)\]/g) || [];
              ...
              return (
                <React.Fragment key={`section-${index}`}>
                  ...
                  <div className={styles.inlineSideEventContainer}>
                    <div className={styles.sideEvent}>
                      <button
                        className={styles.sideEventHeader}
                        onClick={() => toggleSideEvent(sideEvent.id)}
                      >
                      ...
                      {expandedSideEvents[sideEvent.id] && (
                        <div className={styles.sideEventContent}>
                          {(() => {
                            const parts =
                              sideEvent.description.split(/\[TABLE-\d+\]/);
                            const markers =
                              sideEvent.description.match(/\[TABLE-\d+\]/g) ||
                              [];
                              ...
                              if (idx < markers.length) {
                                ...
                                if (tableMatch && event.tables) {
                                  ...
                                  if (tableIndex < event.tables.length) {
                                    return (
                                      <React.Fragment
                                        key={`side-${sideEventIndex}-table-${tableIndex}`}
                                      >
                                        {parsedContent}
                                        <div className={styles.tableWrapper}>
                                          <EventTable
                                            table={event.tables[tableIndex]}
                                          />
                                        </div>
                                      </React.Fragment>
                                    );

...
export const getSubtextContent = (text: string): string =>
  text.match(patterns.subtext)?.[1] || text;

export const isHeadertext = (text: string): boolean =>
  patterns.headertext.test(text);
export const getHeadertextContent = (text: string): string =>
  text.match(patterns.headertext)?.[1] || text;

export const isImage = (text: string): boolean => patterns.image.test(text);
export const isVideo = (text: string): boolean =>
  patterns.video.test(text) || patterns.youtube.test(text);

export const getClass = (
  type: "subtext" | "headertext" | "regular"
): string => {
  switch (type) {
    case "subtext":
      return "subtext";
...
export type { Link, VideoDetails, ImageDetails, EventLink };

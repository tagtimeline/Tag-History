...
export default function AdminCategories() {
  ...
  const [categoryForm, setCategoryForm] = useState<Category>(initialCategoryForm);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const cleanData = {
        ...
        ...(categoryForm.extraStyles && Object.keys(categoryForm.extraStyles).length > 0 ? 
          ...
          <div className={baseStyles.formSection}>
            ...
            <div className={categoriesStyles.categoriesList}>
              {categories.map(category => (
                <div 
                  key={category.id} 
                  className={`${categoriesStyles.categoryItem} ${selectedCategory?.id === category.id ? categoriesStyles.selected : ''}`}
